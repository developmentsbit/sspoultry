...
<div class="content-wrapper">
	<div class="page-heading">
		<h3 class="page-title">Asset Depreciations</h3>
	</div>
	...
	<div class="page-content fade-in">
		<div class="ibox">
			<div class="ibox-head mb-3 myhead">
				<div class="ibox-title"><i class="fa fa-list-ul" aria-hidden="true"></i>&nbsp;&nbsp;Asset Depreciations List</div>
				...
				<table class="table table-striped table-bordered" id="example-table" cellspacing="0" width="100%">
					...
					<tbody class="tbody" id="showtdata">
						...
						<tr id="tr{{ $d->id }}">
							<td>{{ $i++ }}</td>
							<td>{{ $d->asset_title }}</td>
							<td>{{ $d->depreciation_value_per }}</td>
							<td>{{ $d->description }}</td>
							<td>
		...
		<form method="post" class="btn-submit">
			...
			<div class="modal-content rounded">
				<div class="modal-header bg-dark text-light">
					<h5 class="modal-title" id="exampleModalCenterTitle"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Asset Category</h5>
				...
				<div class="modal-body myinput">
					...
					<div class="row">
						...
						<div class="form-group col-md-12">
							...
							<div class="input-group">
								...
								<select class="form-control w-100" name="asset_title_id" style="width:100%!important">
								    ...
								    @if(isset($title))
								    @foreach($title as $t)
								     <option value="{{ $t->id }}">{{ $t->asset_title }}</option>
								    @endforeach
								    @endif
						...
						<div class="form-group col-md-12">
							...
							<div class="input-group">
								<div class="input-group-addon"><i class="fa fa-text-width"></i></div>
								<input class="form-control" type="text" name="depreciation_value_per" id="depreciation_value_per" placeholder="Depreciation Value" required="">
							</div>
						</div>
						...
						<div class="form-group col-md-12">
							...
							<div class="input-group">
								<div class="input-group-addon"><i class="fa fa-text-width"></i></div>
								<input class="form-control" type="text" name="description" id="description" placeholder="Details">
							</div>
						</div>
...
<div class="modal fade" id="exampleModalCenters" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitles" aria-hidden="true">
	<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
		...
		<div class="modal-content rounded">
			<div class="modal-header bg-dark text-light">
				<h5 class="modal-title" id="exampleModalCenterTitles"><i class="fa fa-plus"></i>&nbsp;&nbsp;Edit</h5>
				<button type="button" class="close text-light" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script type="text/javascript">
	...
	$(".btn-submit").submit(function(e){
		...
		$.ajax({
			url:'{{ url('assetdepreciationinsert') }}',
			method:'POST',
			data:data,
	...
	$(".edit").click(function(){
		...
		$.ajax(
		{
			url: "{{ url('editassetdepreciation') }}/"+id,
			type: 'get',
			data:{},
			success: function (data)
			{
	...
	$(".delete").click(function(){
		...
		.then((willDelete) => {
			if (willDelete) {
				...
				$.ajax(
				{
					url: "{{ url('deleteassetdepreciation') }}/"+id,
					type: 'get',
					success: function()
					{
						$('#tr'+id).hide();
