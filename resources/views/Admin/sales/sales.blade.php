							...
							<div class="row">
								<div class="form-group col-md-12">
									...
									<div class="input-group">
										...
										@php
										 $purchasereturn = DB::table('return_purchase_entry')->where('product_id',$i->pdt_id)->sum('product_quantity');
				                         $salesreturn    = DB::table('sales_return_entry')->where('product_id',$i->pdt_id)->sum('return_quantity');
										$stockqty = (($i->quantity+$salesreturn)-( $i->sales_qty+$purchasereturn));
										@endphp
			...
			<div class="col-md-3">
				...
				<div class="col-md-12 bg-light p-3">
						...
						<div class="input-group">
							...
							<input type="text" id="getcustomerpreviousdue" name="getcustomerpreviousdue" class="form-control"  readonly="">
		...
		<div class="col-12 border p-4 mt-4">
			<center>
				...
				<div class="col-md-4 mb-4">
					<div class="form-group">
						<label><b>Sales Type:</b> <span class="text-danger">*</span></label>
						<div class="input-group">
							<select class="form-control" name="typesales" id="typesales">
								<option value="1">Confirm Sales</option>
								<option value="2">Pos Print</option>
								<option value="3">Draft Sales</option>
...
<script type="text/javascript">
	...
	function getcustomerphone(){
		...
		getcustomerpreviousdue();

	}



function getcustomerpreviousdue(){
		...
		$.ajax({
			...
			success: function (response)
			{
				$("#getcustomerpreviousdue").val(response);
			},
			error:function(errors){
				alert("Select Customer")
			}
	...
	function showsalesproductcart(){
		$.ajax({
			...
			success: function (data)
			{
				...
				$("#grandtotal").val((totalsalesamount+vattotal).toFixed(2));
	...
	$(".btn-submit").submit(function(e){
		...
		$.ajax({
			...
			success:function(response){
				...
				if (typesales == 1) {
					window.open('{{URL::to('/invoicesalesa4')}}'+'/'+response, "_blank");
					location.reload();

				}
				else if(typesales == 2){
					window.open('{{URL::to('/invoicesales')}}'+'/'+response, "_blank");
					location.reload();

				}
				else{
					window.open('{{URL::to('/invoicesalesa42')}}'+'/'+response, "_blank");
					location.reload();
