...
<body>
  ...
  <div class="invoice">
    ...
    <table class="table table-bordered">
      ...
      <!-- <thead> -->
       ...
       <tbody>
        ...
        @php
          $total_amount =($total_amount+($p->product_return_price*$p->return_quantity))-($p->return_discount_amount*$p->return_quantity);
        @endphp

        <tr>
          <td>{{ $i++ }}</td>
          <td>{{ $p->pdt_name_en }} {{ $p->pdt_name_bn }}</td>
          <td>{{ $p->return_quantity }}</td>
          <td>{{ $p->product_return_price }}</td>
          <td>{{ $p->return_discount_amount}}</td>
          <td>{{ ($p->product_return_price*$p->return_quantity)-($p->return_discount_amount*$p->return_quantity) }}</td>
        </tr>
      ...
      <tr>
        ...
        <td>
          ...
          {{ ($total_amount-$data->final_discount) }} /-<br>
    ...
    <span class="note p-4">
      <span style="text-transform: capitalize;"><b>In Word:</b> {{ $numberTransformer->toWords($total_amount-$data->final_discount) }} Taka Only.</span>
    </span>
