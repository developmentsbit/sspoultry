...
<div class="content-wrapper">
	<div class="page-heading">
		<h3 class="page-title">Draft Sales Payment List</h3>
	</div>
	...
	<div class="page-content fade-in">
		<div class="ibox">
			<div class="ibox-head mb-3 myhead">
				<div class="ibox-title"><i class="fa fa-list-ul" aria-hidden="true"></i>&nbsp;&nbsp;Draft Sales Payment List</div>
			...
			<div class="ibox-body table-responsive overflow">
				<table class="table table-striped table-bordered" id="example-table" cellspacing="0" width="100%">
					...
					<tbody class="tbody" id="showtdata">
						...
						@if(isset($data))
						@foreach($data as $d)
						@if($d->payment_amount > 0 || $d->discount > 0)
						<tr id="tr{{ $d->id }}">
							<td>{{ $i++ }}</td>
							<td>{{ $d->entry_date  }}</td>
							<td>{{ $d->entry_date  }}</td>
							...
							<td>
								...
								<a  onclick="return confirm('Are you Sure?')" href="{{ url('confirmdraftsalespayment/'.$d->id) }}"  class="btn btn-success  border-0 text-light">Confirm Now</a>
	...
	$(".delete").click(function(){
		...
		.then((willDelete) => {
			if (willDelete) {
				...
				$.ajax(
				{
					url: "{{ url('deletedraftsalespayment') }}/"+id,
					type: 'get',
					success: function()
					{
						$('#tr'+id).hide();
