...
<div class="content-wrapper">
	<div class="page-heading">
		<h3 class="page-title">Purchase with Sales Information</h3>
	</div>
	...
	<div class="page-content fade-in">
		<div class="ibox">
			<div class="ibox-head mb-3 myhead">
				<div class="ibox-title"><i class="fa fa-list-ul" aria-hidden="true"></i>&nbsp;&nbsp;Add Purchase With Sales</div>
			</div>
			<div class="ibox-body">
				...
				<!--			<input type="text" name="barcode" id="barcode" class="form-control" placeholder="Product Barcode" onchange="productbarcodeaddps()">-->
				<!--			<button type="button" class="border text-success" style="cursor: pointer;" onclick="productbarcodeaddps()" title="Add Product"><i class="fa fa-refresh"></i></button>-->
				<!--		</div>-->
				<!--	</div>-->
				<!--</div>-->

				
				<form method="post" action="{{ url("purchasesalesledger") }}" class="reloadform myinput">
					@csrf
					...
					<div class="col-md-12 p-0 row">
						...
						<div class="form-group col-md-4">
							...
							<div class="input-group">
								...
								<select class="form-control select2_demo_1" name="tcustomer_id" id=
								"tcustomer_id" required="" >
								@php
					...
					<div class="form-group col-md-4">
						<label>Customer Name: (3rd Party) <span class="text-danger" style="font-size: 15px;">*</span></label>
						<div class="input-group">
							<div class="input-group-addon"><i class="fa fa-user"></i></div>
							...
							@php
							$customer = DB::table('customer_info')->where("Customer_branch_id",Auth('admin')->user()->branch)->where("type",3)->get();		
							@endphp 
							@foreach($customer as $i)
							<option value="{{ $i->customer_id  }}">{{ $i->customer_id }} - {{ $i->customer_name_en }}</option>
							@endforeach
				...
				<div class="form-group col-md-4">
					...
					<div class="input-group">
						...
						<input type="date" name="invoice_date" placeholder="Invoice Date" class="form-control" required="" autocomplete="off" value="{{ date('Y-m-d') }}">
							...
							<div class="row">
								<div class="form-group col-md-12">
									...
									<div class="input-group">
										...
										"pdt_id"  onchange="return purchasesalesproductcart()">
										<option value="">Select Product</option>
										@php
										$product = DB::table('pdt_productinfo')
										->where("pdt_status",1)
										->get();
		...
		<div class="col-12 border p-4 mt-4">
			<center>
				...
				<input type="submit" name="submitbutton" value="Submit Now" class="btn btn-success" style="width: 150px; font-weight: bold; border-radius: 30px;">
...
<script type="text/javascript">
	...
	function purchasesalesproductcart(){
		let pdt_id = $("#pdt_id").val();

		$.ajax({
			url: "{{ url('purchasesalesproductcart') }}/"+pdt_id,
			type: 'GET',
			success: function (data)
			{
	...
	function showpurchasesalesproductcart(){
		$.ajax({
			url: "{{ url('showpurchasesalesproductcart') }}",
			type: 'get',
			data:{},
			success: function (data)
			{
				...
				let totalpurchaseamount = parseFloat($("#totalpurchaseamounts").val())
				$("#totalamount").val(totalsalesamount.toFixed(2));
				$("#totalpurchaseamount").val(totalpurchaseamount.toFixed(2));
	...
	function purchasesalepriceupdatesingle(id){
		...
		$.ajax({
			url: "{{ url('purchasesalepriceupdatesingle') }}/"+id,
			type: 'GET',
	...
	function purchasepriceupdatesingle(id){
		...
		$.ajax({
			url: "{{ url('purchasepriceupdatesingle') }}/"+id,
			type: 'GET',
			data:{purchase_price_per_unit:purchase_price_per_unit},
			success: function (data)
			{
				Command:toastr["success"]("Product Quentity Update")
				toastr.options = {
	...
	function qtyupdatesales(id){
		...
		$.ajax({
			url: "{{ url('qtyupdatesalespurchase') }}/"+id,
			type: 'POST',
	...
	function productbarcodeaddps(){

		var barcode = $("#barcode").val();

		$.ajax({
			url: "{{ url('purchasesalesproductcart2') }}/"+barcode,
			type: 'GET',
			success: function (data)
			{
				Command:toastr["success"]("Product Added Successfully Done")
