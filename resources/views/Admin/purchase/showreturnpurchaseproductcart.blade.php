		...
		<tr id="tr{{ $d->id }}">
			...
			<td>
				<div class="input-group">
					<input type="text" name="purchase_price" id="purchase_price{{ $d->id }}" class="form-control"  value="{{ $d->purchase_price }}" onchange="returnpurchasepriceupdate('{{ $d->id }}')">
					<button type="button" class="border text-success" style="cursor: pointer;" onclick="returnpurchasepriceupdate('{{ $d->id }}')" title="Update Price"><i class="fa fa-refresh"></i></button>
			...
			$(".delete").click(function(){
				...
				.then((willDelete) => {
					if (willDelete) {
						...
						$.ajax(
						{
							url: "{{ url('deletereturnpurchasecartproduct') }}/"+id,
							type: 'get',
							success: function()
							{
								$('#tr'+id).hide();
