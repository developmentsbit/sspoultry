			...
			<div class="ibox-body table-responsive overflow">
				<table class="table table-striped table-bordered" id="example-table" cellspacing="0" width="100%">
					...
					<tbody class="tbody">
						...
						$return       = DB::table("supplier_payment")->where("suplier_id",$d->supplier_id)->sum('return_amount');
						$pd       = DB::table("supplier_payment")->where("suplier_id",$d->supplier_id)->where("comment","PD")->sum('previous_due');
						$due        = ($grandtotal+$pd)-$paid;

						$duetotal = ($duetotal+$pd+$due)-$return;
						@endphp
