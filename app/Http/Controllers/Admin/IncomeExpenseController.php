...
class IncomeExpenseController extends Controller
{
	...
	public function income_expensetitleinsert(Request $r){
		...
		$id = IdGenerator::generate(['table' => 'income_expense_title', 'field'=>'id','length' => 10, 'prefix' =>'INE-']);
...
// End Title
	...
	public function incomeentryinsert(Request $r){
		...
		$id = IdGenerator::generate(['table' => 'income_entry', 'field'=>'id','length' => 10, 'prefix' =>'INC-']);
...
// End Income Entry
	...
	public function expenseentryinsert(Request $r){
		...
		$id = IdGenerator::generate(['table' => 'expense_entry', 'field'=>'id','length' => 10, 'prefix' =>'EXP-']);
...
// End Income Entry
	...
	public function librality(){
		$data = DB::table('librality')
		->join("income_expense_title","income_expense_title.id","librality.expense_id")
		->select("librality.*","income_expense_title.title")
		->where("librality.branch",Auth('admin')->user()->branch)
		->get();
		return view('Admin.incomeexpense.librality',compact('data'));
	}

	public function libralityinsert(Request $r){

		$id = IdGenerator::generate(['table' => 'librality', 'field'=>'id','length' => 10, 'prefix' =>'LIB-']);
		...
		DB::table('librality')->insert($data);
	}

	

	public function deletelibrality($id){

		DB::table('librality')->where('id',$id)->delete();
	}

	public function getlibrality(){
		$data = DB::table('librality')
		->join("income_expense_title","income_expense_title.id","librality.expense_id")
		->select("librality.*","income_expense_title.title")
		->where("librality.branch",Auth('admin')->user()->branch)
		->get();
		return view('Admin.incomeexpense.getlibrality',compact('data'));
	}


	public function editlibrality($id){

		$data = DB::table('librality')->where('id',$id)->first();
		return view('Admin.incomeexpense.editlibrality',compact('data'));
	}


	public function updatelibrality(Request $r,$id){
		...
		DB::table('librality')->where('id',$id)->update($data);
	}
